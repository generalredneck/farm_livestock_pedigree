...
function set_vars() {
  ...
  offset = $(".farm-pedigree").offset();

  console.log(offset);
  w = current_width;
  h = current_height - offset.top - 5;
...
function setup() {
  ...
  json = Drupal.settings.farm_livestock_pedigree.pedigree_data;
  ...
  if (ancestorRoot._parents !== undefined) {
    ancestorRoot._parents.forEach(function(parents){
      if (parents._parents !== undefined) {
        // 2
        parents._parents.forEach(function(parents){
          if (parents._parents !== undefined) {
            // 3
            parents._parents.forEach(function(parents){
              if (parents._parents !== undefined) {
  ...
  if (ancestorRoot._children !== undefined) {
...
Tree.prototype.drawNodes = function(nodes, source){
  ...
  nodeUpdate.select('text.breed')
      .attr("dx", -(boxWidth/2) + 10)
      .style('fill-opacity', 1);
  nodeUpdate.select('text.dob')
      .attr("dx", -(boxWidth/2) + 10)
      .style('fill-opacity', 1);
